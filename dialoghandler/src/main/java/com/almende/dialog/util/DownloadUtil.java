...
public class DownloadUtil {
	...
	@POST
	@Path("/{file}")
	public Response download(@PathParam("file") String filename, @QueryParam("blob-key") String keyValue, @Context HttpServletResponse res, String data) {
		
		MyBlobStore store = new MyBlobStore();
		if(filename.equals("audio.vxml")) {
			filename = store.getFileName(new BlobKey(keyValue));
			String resp = "<?xml version=\"1.0\" encoding=\"UTF-8\"?> "
					...
					+ "<var name=\"url\" expr=\"'http://keyserver.ask-fast.com/dialoghandler/download/rest/"+filename+"'\"/>"
